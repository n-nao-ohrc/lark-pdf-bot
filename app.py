...
- 各要素は必ず以下のキーを持つ:
  ...
  - 数量: number
  - 単価: number
  - 税区分: string   # "税込" または "税抜"
- 商品名・数量・単価が取れない行は除外
- 小計、合計、内消費税、外消費税、送料単独行、住所、電話番号、会社情報、見積番号などは除外
- 明細と判断できる行だけ抽出
- 金額欄と数量欄があり、単価が明示されていない場合は、単価 = 金額 / 数量 として整数化してよい
- 「税抜」と明示されている場合のみ 税区分="税抜"
- それ以外はすべて 税区分="税込" とする
- 数値はカンマや円記号を除いた number で出力する
- 日本語はそのままでよい
...
""".strip()
    ...
    for item in items:
        ...
        unit_price = int(float(item["単価"]))
        tax_category = str(item.get("税区分", "税込")).strip()
...
def root():
    ...
    try:
        ...
        if not all(config.values()):
            return jsonify({
                "error": "tenant config incomplete",
                "tenant": tenant_key,
                "config": config
            }), 500
